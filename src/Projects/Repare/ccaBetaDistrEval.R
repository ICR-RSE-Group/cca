...
pos_scores <- subset(scores, scores >= 0)
n <- min(topN, length(pos_scores))
top_scores <- pos_scores[1:n]
...
if (header) {
  out_head = c("Shape1\tShape2\tMax_Rank\tMin_Score");
...
if (jenks != "") {
  ...
  jenks_scores <- pos_scores[1:top_jenks]
